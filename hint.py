...
parser.add_argument('--step_size', '-dx', type=int, metavar='step size', default=0.01, help='The step size used for the finite differences calculation, written as a percentage of the feature range')
parser.add_argument('--threshold', '-t', type=int, metavar='threshold', default=3, help='The threshold for the local interaction feature pair ranking, in units of standard deviations of the Hessian matrix')
parser.add_argument('--name', '-i', type=int, metavar='unique_name', default=0, help='Saves the run under a specific name')
...
def main():
  ...
  if (args.rank_local):
    utils.rank_local(model, args.number_of_core_samples, args.step_size, args.name, args.threshold, args.plot, args.output_path)
